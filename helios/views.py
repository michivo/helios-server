...
def _election_vote_shortcut(request, election):
  vote_url = "%s/booth/vote.html?%s" % (settings.SECURE_URL_HOST, urllib.parse.urlencode({'election_url' : reverse(url_names.election.ELECTION_HOME, args=[election.uuid])}))
  
  test_cookie_url = "%s?%s" % (reverse(url_names.COOKIE_TEST), urllib.parse.urlencode({'continue_url' : vote_url}))
...
def test_cookie(request):
  ...
  next_url = "%s?%s" % (reverse(url_names.COOKIE_TEST_2), urllib.parse.urlencode({'continue_url': continue_url}))
...
def test_cookie_2(request):
  ...
  if not request.session.test_cookie_worked():
    return HttpResponseRedirect(settings.SECURE_URL_HOST + ("%s?%s" % (reverse(url_names.COOKIE_NO), urllib.parse.urlencode({'continue_url': continue_url}))))

  request.session.delete_test_cookie()
  return HttpResponseRedirect(continue_url)  

def nocookies(request):
  retest_url = "%s?%s" % (reverse(url_names.COOKIE_TEST), urllib.parse.urlencode({'continue_url' : request.GET['continue_url']}))
