...
class ElectionForm(forms.Form):
  short_name = forms.SlugField(max_length=40, help_text='keine Leerzeichen, wird Teil der URL f&uuml;r die Wahl, z.B. landesversammlung-2020')
  name = forms.CharField(max_length=100, widget=forms.TextInput(attrs={'size':60}), help_text='der offizielle Name deiner Wahl, z.B. Landesversammlung 2020')
  ...
  use_voter_aliases = forms.BooleanField(required=False, initial=True, help_text='Wenn angehakt, werden die W&auml;hler*innen-Namen im Tracking-Center durch Aliases (wie "V42") ersetzt')
  #use_advanced_audit_features = forms.BooleanField(required=False, initial=True, help_text='disable this only if you want a simple election with reduced security but a simpler user interface')
  randomize_answer_order = forms.BooleanField(required=False, initial=False, help_text='Wenn angehakt, werden die Wahlm&ouml;glichkeiten den W&auml;hler*innen in zuf&auml;lliger Reihenfolge angezeigt')
  ...
  help_email = forms.CharField(required=False, initial="", label="Hilfe-Kontakt", help_text='Eine Email-Adresse, an die sich W&auml;hler*innen wenden k&ouml;nnen, wenn sie Hilfe brauchen.')
  ...
  # times
  voting_starts_at = SplitDateTimeField(help_text = 'Startdatum/-zeit der Wahl (UTC!)',
                                   widget=SplitSelectDateTimeWidget, required=False)
  voting_ends_at = SplitDateTimeField(help_text = 'Enddatum/-zeit der Wahl (UTC!)',
                                   widget=SplitSelectDateTimeWidget, required=False)

class ElectionTimeExtensionForm(forms.Form):
  voting_extended_until = SplitDateTimeField(help_text = 'Verl&auml;ngertes Enddatum der Wahl (UTC!)',
                                   widget=SplitSelectDateTimeWidget, required=False)
